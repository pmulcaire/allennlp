...
print OUT "  ------------------+------+---------+--------+------------+---------------|--------|\n";
my ($ov_corr, $ov_g, $ov_s, $ov_prec, $ov_rec, $ov_f1) = (0, 0, 0, 'NaN', 'NaN', 'NaN');
foreach my $label (sort keys %{$srl_counts{arg_per_tag}}) {
    ...
    printf OUT "  %-17s | %5d | %7d | %6d | %10s | %13s | %6s |\n",
    $label, $tot_g, $tot_corr, $tot_s, $rec, $prec, $f1;
}

$ov_rec = sprintf("%.2f",$ov_corr / $ov_g * 100);
$ov_prec = sprintf("%.2f",$ov_corr / $ov_s * 100);
if($ov_prec > 0 and $ov_rec > 0){
    $ov_f1 = sprintf("%.2f", 2 * $ov_prec * $ov_rec / ($ov_prec + $ov_rec));
}
printf OUT "  %-17s | %5d | %7d | %6d | %10s | %13s | %6s |\n",
    "Micro-average", $ov_g, $ov_corr, $ov_s, $ov_rec, $ov_prec, $ov_f1;

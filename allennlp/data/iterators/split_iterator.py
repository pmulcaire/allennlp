...
class SplitIterator(BasicIterator):
    """
    An iterator which splits instances into groups according to a provided key. Each group's batches
    are constructed by a sub-iterator. The benefit is ensuring that batches are not merely sorted
    by a key, but guaranteed to contain only one value (and padded with instances from within their
    group if necessary).

    Parameters
    ----------
    split_key : Tuple[str, str], optional (default = [])
    ...
    def __init__(self,
                 split_key: Tuple[str, str] = None,
                 sorting_keys: List[Tuple[str, str]] = None,
                 padding_noise: float = 0.1,
                 biggest_batch_first: bool = False,
                 batch_size: int = 32) -> None:
        self._split_key = split_key or []
        self._sorting_keys = sorting_keys or []
        self._padding_noise = padding_noise
        self._biggest_batch_first = biggest_batch_first
        super(SplitIterator, self).__init__(batch_size)

    @overrides
    def _create_batches(self, dataset: Dataset, shuffle: bool) -> List[List[Instance]]:
        if self._split_key:
            datasets = self._split_by_key(dataset, self._split_key)
        total_batches = []
        for sublist in datasets:
            subdataset = Dataset(sublist)
            # generalization:
            # set a sub_iterator type(s) in __init__
            # total_batches += self.sub_iterator._create_batches(subdataset)
            # done
            # then shuffle at the end, as now
            if self._sorting_keys:
                subdataset = self._sort_dataset_by_padding(subdataset,
                                                           self._sorting_keys,
                                                           self._padding_noise)
            # List[List[instances]]; last might be short; unpadded
            grouped_instances = super(SplitIterator, self)._create_batches(subdataset, shuffle=False)
            ...
            total_batches += grouped_instances
        if shuffle:
    ...
    @staticmethod
    def _split_by_key(dataset: Dataset, key: Tuple[str,str]):
        datasets = {}
        for instance in dataset.instances:
            val = getattr(instance.fields[key[0]], key[1])
            if val not in datasets:
                datasets[val] = []
            datasets[val].append(instance)
    ...
    @classmethod
    def from_params(cls, params: Params) -> 'SplitIterator':
        split_key = params.pop('split_key', '')
        ...
        return cls(split_key=split_key,
